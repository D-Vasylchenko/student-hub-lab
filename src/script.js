...
if (fileInput) {
    fileInput.addEventListener('change', (e) => {
        ...
        if (file) {
            // Перевірка на HEIC (Apple формат)
            if (file.type === 'image/heic' || file.name.toLowerCase().endsWith('.heic')) {
                alert(
                    '⚠️ Формат HEIC не підтримується більшістю браузерів.\nБудь ласка, виберіть фото у форматі JPG або PNG.',
                );
                e.target.value = ''; // Очищаємо вибір
                document.getElementById('profile-edit-preview').src = ''; // Прибираємо прев'ю
                return;
            }
...
async function saveProfile() {
    ...
    // 👇 1. ПЕРЕВІРКИ ФАЙЛУ (БЕЗПЕКА) 👇
    if (file) {
        // Перевірка розміру (2 МБ = 2 * 1024 * 1024 байт)
        if (file.size > 2 * 1024 * 1024) {
            alert('⚠️ Файл занадто великий! Виберіть фото менше 2 МБ.');
            return; // Зупиняємо функцію
        }

        // Перевірка формату (Забороняємо HEIC)
        if (file.type === 'image/heic' || file.name.toLowerCase().endsWith('.heic')) {
            alert('⚠️ Формат HEIC не підтримується браузерами.\nБудь ласка, виберіть JPG або PNG.');
            return; // Зупиняємо функцію
        }
    }
    ...
    try {
        // Якщо файл пройшов перевірку — вантажимо
        if (file) {
            const storageRef = sRef(storage, `avatars/${currentUser}`);
        ...
        alert('Щось пішло не так при завантаженні. Спробуйте інше фото.');
    } finally {
...
function renderMessage(msg) {
    ...
    let senderHtml = isMine
        ? ''
        : `<span class="msg-sender" onclick="openProfileModal('${msg.user}')">${msg.user}</span>`;
    ...
    weatherTimeout = setTimeout(async () => {
        try {
            const res = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${query}&accept-language=uk&limit=5`,
            );
